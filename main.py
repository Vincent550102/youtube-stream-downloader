...
def record_live_stream(url):
    result = subprocess.run([ytarchive_bin, '-w', '--merge', '-o', output_format, '--ffmpeg-path', ffmpeg_bin, url, 'best'], capture_output=True)
    if "Use yt-dlp instead." in result.stderr.decode('utf-8'):
        subprocess.run([yt_dlp_bin, '-o', output_format, '--ffmpeg-location', ffmpeg_bin, url])
    return result
